...
tuple TSportSchedulingParams {
  int nbTeamsInDivision;  // number of teams in one division
  int maxTeamsInDivision; // max nb teams
  int nbIntraDivisional;  // nb of intra-disivional matches to play
  int nbInterDivisional;  // nb of extra-divisional matches to play
}
TSportSchedulingParams scheduleParams = ...;
...
/// INTERNAL DATA
int nbTeamsInDivision  = scheduleParams.nbTeamsInDivision;
int nbIntraDivisional  = scheduleParams.nbIntraDivisional;
int nbInterDivisional  = scheduleParams.nbInterDivisional;
int maxTeamsInDivision = scheduleParams.maxTeamsInDivision;
...
assert card(teamDiv1) >= scheduleParams.nbTeamsInDivision;
assert card(teamDiv2) >= scheduleParams.nbTeamsInDivision;
///
/// -- END DATA ---
...
{teamMapping} teamLeague =
  { <t, item(teamDiv1,t).name> | t in 1..nbTeamsInDivision } 
    union 
  { <t+nbTeamsInDivision, item(teamDiv2,t).name> | t in 1..nbTeamsInDivision};
